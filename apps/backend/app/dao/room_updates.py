...
class RoomUpdateDAO:
    ...
    async def record(self, room_id: str, data: bytes, max_room_bytes: int, min_frame_bytes: int) -> bool:
        ...
        and no room may grow its replay past `max_room_bytes`, with each frame
        charged at least `min_frame_bytes`.
        """
        query = """
            WITH room AS (
                UPDATE rooms SET recorded_bytes = recorded_bytes + GREATEST(octet_length($2::bytea), $4)
                WHERE id = $1 AND is_active AND recorded_bytes + GREATEST(octet_length($2::bytea), $4) <= $3
                RETURNING id
            )
            INSERT INTO room_updates (room_id, data) SELECT id, $2::bytea FROM room
        """
        async with self.pool.acquire() as conn:
            return await conn.execute(query, room_id, data, max_room_bytes, min_frame_bytes) == "INSERT 0 1"
