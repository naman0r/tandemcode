...
def test_a_stalled_peer_is_dropped_and_the_rest_still_hear_chat(monkeypatch):
    ...
    # One deadline the first time, none once the peer is known to be stalled.
    assert first < 0.5 and second < 0.05
...
def test_a_stalled_editor_peer_does_not_hold_the_relay(monkeypatch):
    ...
    asyncio.run(manager.relay_text("r", sender, "hello"))
    assert bob.received == ["hello"] and sender.received == []
    assert stuck.closed
