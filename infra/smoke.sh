...
check "Web serves the app"            "curl -fsS $WEB/ | grep -q '<div id=\"root\">'"
check "Web serves client routes"      "curl -fsS $WEB/rooms/anything | grep -q '<div id=\"root\">'"
check "Web refuses framing"           "curl -fsS -D - -o /dev/null $WEB/ | grep -qi 'x-frame-options: DENY'"
